...
const VotingPage = () => {
  ...
  const mapCandidate = (rawData) => {
    return rawData
      ...
      .map((data) => ({
        ...
        description: data.vision || 'Candidate Vision',
      }));
  };
  ...
  const handleOnSelect = async (vote) => {
    ...
    const { value: email } = await Swal.fire({
      title:
        vote.id === 0
          ? 'กรุณากรอกอีเมลสถาบันเพื่อยืนยันการเลือกไม่ประสงลงคะแนน'
          : `กรุณากรอกอีเมลสถาบันเพื่อยืนยันการเลือกคุณ ${vote.candidateName}`,
      input: 'email',
    ...
    <div>
      ...
      <div className="VTitle">
        ...
        <div className="Votename">
          <h1>ลงคะแนนการเลือกตั้ง {electionName} </h1>
        </div>
      </div>
